...
if(isset($sign))
{
	...
	$check_api=array(
			...
			//'pay_type'=>array(0=>14,1=>get_apiencode("S14","支付类型不存在")),
			//'pay_number'=>array(1=>get_apiencode("S15","支付交易号不能为空")),
			...
			//'ReceiveAreaCode'=>array(1=>get_apiencode('S19','地区编码错误')),
			'mobile'=>array(0=>20,1=>get_apiencode('S20','手机号码格式错误')),
			//'email'=>array(0=>21,1=>get_apiencode('S21','邮箱格式错误')),
			'IDCardNumber'=>array(0=>23,1=>get_apiencode('S23','身份证格式错误必须是18位正确的身份证号')),
		    'IDCardName'=>array(0=>26,1=>get_apiencode('S22','身份证姓名格式不正确'))
		 );
		  
	$check_api_null=array(
			//'ShippingAmount'=>array(0=>24,1=>get_apiencode('S24','运费必须大于等于0的数字')),
			//'TaxAmount'=>array(0=>25,1=>get_apiencode('S25','税必须大于等于0的数字')),
		 );
		 
	//省份证判断
	function openpi_isCnNewID($cid){  
		...
		$idx = $sum % 11;  
		// 检验第18为是否与校验码相等
		return $arrValid[$idx+1].""==strtolower(substr($cid,17,1))."";
	}
	/**
	 * @param $num
	 * @param $str
	...
	foreach($data['ItemList'] as $k=>$v)
	{
	    ...
	    $db->query("select   hg_type,status,warehouse_id    from  ".tab_m('stock')."  where id='$stock_id'   limit 1  ");  
		$stock=$db->fetchRow();
		
		if(empty($stock['warehouse_id']))
			get_apiencode('S31','订阅编码'.$v['ProductID']." 仓库异常",1);
		
		if(empty($stock['hg_type']))
			get_apiencode('S31','订阅编码'.$v['ProductID']." 贸易类型异常",1);
		...
		if(!empty($hg_type)&&$hg_type!=$stock['hg_type'])	
			get_apiencode('S31',$v['ProductID'].' 同一个订单不能包含多个贸易类型的产品，请联系管理员',1);
				
		if(empty($stock['hg_type']))	
			$hg_type=$stock['hg_type']*1;
	...
	{
		$data['pay_number']=mysql_escape_string($data['pay_number']);
		$db->query("select id  from  ".tab_m('order')."  where  cashflow_id='$data[pay_number]' "); 
		$num=$db->fetchField('id');
		if(!empty($num))
			get_apiencode('S32','支付流水号重复',1);
	}
	...
	foreach($data['ItemList'] as $k=>$v)
	{
		...
		if(!empty($d['is_split']))
		{
			if(count($data['ItemList'])>1)
				get_apiencode('S35','平台编号['.$de['stock_id'].']  只能独立销售',1);
		}
	...
	$money=$price+$sum_weight_price+$price_rate;//加运费
	 ...
	 if(empty($check_no_card))
	 {	
		 $payment_name=array(2=>'支付宝',3=>'微信支付',4=>'财付通',5=>'银联支付');
		 $payment_type=array(112=>2,118=>3,114=>4,117=>5);	
		 $paytype=$payment_type[$data['pay_type']];
		 $paytypestr=$payment_name[$paytype];	
		 $data['email']="1";	
		 $payment_name=$paytypestr;
		 $payment_type=$paytype;
		 $cashflow_id=$data['pay_number'];
		 $card_name=$data['IDCardName'];
		 $card_no=$data['IDCardNumber'];
	 }
	 else
	 {	
		 $paytype=2;
	 ...
	 */
	 ...
	 $sql="INSERT INTO ".tab_m('order_ls')." 
	...
	foreach($data['ItemList'] as $k=>$v)
	{
		...
		$sql="INSERT INTO ".tab_m('order_pro_ls')." 
