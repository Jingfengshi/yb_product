...
function test_api($mothed,$type='')
{
	...
	if($mothed=='search.order')
		$data['order_id']='201603291011155';//
    
	//------------订单创建-------------
	if($mothed=='create.order')
	{
        $post['method']='create.order';
		$data['create_time']='20160531000000'; //渠道订单  必选
		$data['order_id']='201706210022'; //渠道订单  必选
		$data['pay_type']='112';  //支付类型 必选 支付方式编号 111: 东方支付 112:支付宝 114:财付通 117:银联支付 118: 微信支付
		$data['pay_number']='2016156632222904287234844442223'; //支付号
		$data['payment_money']='220'; //销售总金额
		$data['ReceiveName']='王二强';  //收货人姓名
		$data['ReceiveAreaName']='北京市,北京市,海淀区';//String(100) 必选 收件省市区名称（省市区名称之间用半角有逗号,隔开，如 上海,上海市,静安区）
		...
		$data['IDCardName']='王阿';  //身份证姓名
		$data['company']='XX分销商';  //身份证姓名
		$data['IDCardNumber']='370602197901031324'; //身份证号码必须是二代身份证
		$data['ItemList'][2]['ProductID']='10000019';// String(20) 必选 对接产品编码
		$data['ItemList'][2]['Productname']='测试产品';// String(20) 必选 产品名称
		$data['ItemList'][2]['Quantity']='2';// Number(16) 必选 购买数量
		$data['ItemList'][2]['price']='100';//销售单价
	...
	//------------单品查询-------------
	if($mothed=='search.product')
		$data['ProductID']='18';
		
    //------------订单查询-------------
	if($mothed=='search.orderlist')
	{
	...
	header('Content-Type: text/html; charset=utf-8');
	echo $json=@HttpClient::quickPost("http://www.test-zjh_product.com/spapi/zjx_51jiancang.php",$post);
